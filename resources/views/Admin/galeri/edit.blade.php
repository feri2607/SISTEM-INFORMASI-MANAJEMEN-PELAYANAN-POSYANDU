...
<div class="p-6 max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Foto Galeri</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui informasi foto galeri.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.galeri.update', $galeri) }}" method="POST" enctype="multipart/form-data"
        ...
        <div>
            ...
            @if($galeri->foto)
                <img src="{{ asset('storage/galeri/' . $galeri->foto) }}" alt="{{ $galeri->judul }}"
                     class="h-40 w-auto rounded-xl object-cover mb-3">
            @else
                <p class="text-xs text-gray-400 mb-3">Belum ada foto.</p>
            @endif
            ...
            <img id="foto_preview" class="mt-3 h-40 w-auto rounded-xl object-cover hidden" alt="preview">
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="3"
                      class="w-full rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 px-4 py-2.5 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-transparent">{{ old('deskripsi', $galeri->deskripsi) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="flex items-end pb-2">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ $galeri->is_active ? 'checked' : '' }}
