...
@section('title', 'Edit Balita - Sistem Informasi Posyandu')

@section('page-title', 'Edit Balita')
@section('page-subtitle', 'Perbarui data balita')

@section('content')
    <div class="max-w-5xl mx-auto">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <div class="p-6">
                <form method="POST" action="{{ route($routePrefix . 'update', $balita) }}" id="balitaEditForm"
                    x-data="balitaEditForm()" @submit.prevent="submitForm">
                    @csrf
                    @method('PUT')

                    <!-- Data Balita -->
                    <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Edit Data Balita</h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="warga_id" id="warga_id" required
                                    ...
                                    @foreach($wargaList as $warga)
                                        <option value="{{ $warga->id }}" {{ (old('warga_id') ?? $balita->warga_id) == $warga->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text" name="nama" id="nama" required
                                    class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white"
                                    placeholder="Nama lengkap balita" value="{{ old('nama') ?? $balita->nama }}">
                                @error('nama')
                                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="anak_ke" id="anak_ke"
                                    class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white"
                                    placeholder="Contoh: 1" value="{{ old('anak_ke') ?? $balita->anak_ke }}">
                                @error('anak_ke')
                                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="berat_lahir" id="berat_lahir"
                                    class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white"
                                    placeholder="Contoh: 3.2" value="{{ old('berat_lahir') ?? $balita->berat_lahir }}">
                                @error('berat_lahir')
                                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.1" name="panjang_lahir" id="panjang_lahir"
                                    class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white"
                                    placeholder="Contoh: 50" value="{{ old('panjang_lahir') ?? $balita->panjang_lahir }}">
                                @error('panjang_lahir')
                                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="keterangan" id="keterangan" rows="3"
                                    class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white"
                                    placeholder="Catatan tambahan tentang balita (opsional)">{{ old('keterangan') ?? $balita->keterangan }}</textarea>
                                @error('keterangan')
                                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
    ...
    @push('scripts')
        <script>
            function balitaEditForm() {
                return {
                    loading: false,
                    submitForm() {
                        this.loading = true;
                        document.getElementById('balitaEditForm').submit();
                    }
                }
            }
        </script>
