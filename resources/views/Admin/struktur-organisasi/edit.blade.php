...
<div class="p-6 max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.struktur-organisasi.index') }}" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Pengurus</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui data pengurus di struktur organisasi.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.struktur-organisasi.update', $struktur_organisasi) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="nama" value="{{ old('nama', $struktur_organisasi->nama) }}"
            ...
            <div>
                ...
                <input type="text" name="jabatan" value="{{ old('jabatan', $struktur_organisasi->jabatan) }}"
                       class="w-full rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 px-4 py-2.5 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-transparent">
        ...
        <div>
            ...
            @if($struktur_organisasi->foto)
                <img src="{{ asset('storage/struktur/' . $struktur_organisasi->foto) }}" alt="{{ $struktur_organisasi->nama }}"
                     class="h-24 w-24 rounded-full object-cover mb-3 ring-2 ring-teal-100">
            @else
                <img src="https://ui-avatars.com/api/?name={{ urlencode($struktur_organisasi->nama) }}&color=7F9CF5&background=EBF4FF&size=100"
                     alt="{{ $struktur_organisasi->nama }}" class="h-24 w-24 rounded-full mb-3">
            @endif
            ...
            <img id="foto_preview" class="mt-3 h-24 w-24 rounded-full object-cover hidden" alt="preview">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Deskripsi / Keterangan</label>
            <textarea name="deskripsi" rows="3"
                      class="w-full rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 px-4 py-2.5 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-transparent">{{ old('deskripsi', $struktur_organisasi->deskripsi) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="flex items-end pb-2">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ $struktur_organisasi->is_active ? 'checked' : '' }}
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="{{ route('admin.struktur-organisasi.index') }}" class="px-6 py-2.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-xl hover:bg-gray-200 transition">
                Batal
            </a>
        </div>
    </form>
