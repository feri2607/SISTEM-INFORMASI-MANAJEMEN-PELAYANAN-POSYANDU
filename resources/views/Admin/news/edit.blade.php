...
@section('title', 'Edit Berita - Sistem Informasi Posyandu')

@section('page-title', 'Edit Berita')
@section('page-subtitle', 'Perbarui informasi berita')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="excerpt" id="excerpt" rows="3"
                                  class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white"
                                  placeholder="Ringkasan berita...">{{ old('excerpt', $news->excerpt) }}</textarea>
                        @error('excerpt')
                            <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            <div class="flex-shrink-0">
                                <div class="w-32 h-32 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600">
                                    <img id="thumbnailPreview"
                                         src="{{ $news->thumbnail ? Storage::url('news/' . $news->thumbnail) : 'https://placehold.co/300x200/e2e8f0/64748b?text=Thumbnail' }}"
                                         alt="Preview"
                    ...
                    <div>
                        ...
                        @if($news->gallery)
                            <div class="mb-3 flex flex-wrap gap-2">
                                @foreach(json_decode($news->gallery, true) ?? [] as $img)
                                    <div class="relative w-20 h-20 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-600">
                                        <img src="{{ Storage::url('news/gallery/' . $img) }}" class="w-full h-full object-cover" alt="Galeri">
                                    </div>
                                @endforeach
                            </div>
                            <p class="text-xs text-yellow-600 dark:text-yellow-400 mb-2">⚠️ Upload galeri baru akan menggantikan galeri yang ada.</p>
                        @endif
...
<script>
    function newsEditForm() {
        return {
            ...
            slugEdited: false,
            form: {
                title: '{{ old('title', addslashes($news->title)) }}',
                slug: '{{ old('slug', addslashes($news->slug)) }}',
