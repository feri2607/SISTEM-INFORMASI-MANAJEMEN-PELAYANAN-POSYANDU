{{-- resources/views/pegawai/lansia/pemeriksaan-create.blade.php --}}

@extends('layouts.app')

@section('title', 'Input Pemeriksaan Lansia - Sistem Informasi Posyandu')
@section('page-title', '🩺 Input Pemeriksaan Lansia')
@section('page-subtitle', 'Catat hasil pemeriksaan kesehatan lansia.')

@section('content')
<div class="max-w-3xl mx-auto space-y-6">

    <div class="flex items-center gap-3">
        <a href="{{ route('pegawai.lansia.show', $lansia) }}" 
        ...
        <h2 class="text-xl font-bold text-gray-900 dark:text-white">Form Pemeriksaan: {{ $lansia->nama }}</h2>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="p-6">
            <form action="{{ route('pegawai.lansia.pemeriksaan.store') }}" method="POST" x-data="pemeriksaanLansiaForm()">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="keluhan" id="keluhan" value="{{ old('keluhan') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <div class="flex rounded-md shadow-sm">
                            <input type="number" name="lingkar_perut" step="0.1" min="0" value="{{ old('lingkar_perut') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <div class="flex rounded-md shadow-sm">
                            <input type="number" name="gula_darah" step="0.1" min="0" value="{{ old('gula_darah') }}"
                                   class="flex-1 rounded-l-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-teal-500 focus:border-teal-500 text-gray-900 dark:text-white">
                    ...
                    <div>
                        ...
                        <div class="flex rounded-md shadow-sm">
                            <input type="number" name="kolesterol" step="0.1" min="0" value="{{ old('kolesterol') }}"
                                   class="flex-1 rounded-l-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-teal-500 focus:border-teal-500 text-gray-900 dark:text-white">
                            <span class="inline-flex items-center px-3 rounded-r-md border border-l-0 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-600 text-gray-500 dark:text-gray-300 sm:text-sm">mg/dL</span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex rounded-md shadow-sm">
                            <input type="number" name="asam_urat" step="0.1" min="0" value="{{ old('asam_urat') }}"
                                   class="flex-1 rounded-l-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-teal-500 focus:border-teal-500 text-gray-900 dark:text-white">
                            <span class="inline-flex items-center px-3 rounded-r-md border border-l-0 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-600 text-gray-500 dark:text-gray-300 sm:text-sm">mg/dL</span>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <textarea name="catatan" id="catatan" rows="3" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-teal-500 focus:border-teal-500 text-gray-900 dark:text-white">{{ old('catatan') }}</textarea>
                    </div>
                    <div>
                        <label for="tindakan" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Tindakan / Edukasi</label>
                        <textarea name="tindakan" id="tindakan" rows="3" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-teal-500 focus:border-teal-500 text-gray-900 dark:text-white">{{ old('tindakan') }}</textarea>
                    </div>
                </div>

                <div class="flex justify-end gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <a href="{{ route('pegawai.lansia.show', $lansia) }}" class="px-5 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">Batal</a>
                    <button type="submit" class="px-5 py-2.5 bg-[#036672] text-white rounded-lg hover:bg-[#036672] transition font-medium">Simpan Pemeriksaan</button>
                </div>
            </form>
        </div>
    </div>
...
@push('scripts')
<script>
    function pemeriksaanLansiaForm() {
        return {
            tb: {{ old('tinggi_badan') ?? 'null' }},
            bb: {{ old('berat_badan') ?? 'null' }},
            imt: null,
            statusImt: '',
