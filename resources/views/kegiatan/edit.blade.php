...
@section('title', 'Edit Kegiatan - Sistem Informasi Posyandu')

@section('page-title', 'Edit Kegiatan')
@section('page-subtitle', 'Perbarui data kegiatan Posyandu')

@section('content')
    <div class="max-w-3xl mx-auto">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <div class="p-6">
                <form method="POST" action="{{ route(auth()->user()->role . '.kegiatan.update', $kegiatan) }}" id="kegiatanForm" x-ref="form"
                    x-data="kegiatanForm()" @submit.prevent="submitForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input type="time" name="jam_mulai" id="jam_mulai"
                                    class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white"
                                    value="{{ old('jam_mulai', is_string($kegiatan->jam_mulai) ? date('H:i', strtotime($kegiatan->jam_mulai)) : (optional($kegiatan->jam_mulai)->format('H:i') ?? '08:00')) }}">
                                @error('jam_mulai')
                                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="time" name="jam_selesai" id="jam_selesai"
                                    class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white"
                                    value="{{ old('jam_selesai', is_string($kegiatan->jam_selesai) ? date('H:i', strtotime($kegiatan->jam_selesai)) : (optional($kegiatan->jam_selesai)->format('H:i') ?? '12:00')) }}">
                                @error('jam_selesai')
                                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="lokasi" id="lokasi"
                                    class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white"
                                    placeholder="Masukkan lokasi kegiatan" value="{{ old('lokasi', $kegiatan->lokasi) }}">
                                @error('lokasi')
                                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select name="status" id="status"
                                    class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 dark:text-white">
                                    <option value="terjadwal" {{ old('status', $kegiatan->status) == 'terjadwal' ? 'selected' : '' }}>Terjadwal
                                    </option>
                                    <option value="berlangsung" {{ old('status', $kegiatan->status) == 'berlangsung' ? 'selected' : '' }}>
                                        Berlangsung</option>
                                    <option value="selesai" {{ old('status', $kegiatan->status) == 'selesai' ? 'selected' : '' }}>Selesai
                                    </option>
                                    <option value="dibatalkan" {{ old('status', $kegiatan->status) == 'dibatalkan' ? 'selected' : '' }}>
                                        Dibatalkan</option>
                                </select>
                                @error('status')
                                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
        ...
        <script>
            function kegiatanForm() {
                return {
                    loading: false,
                    form: {
                        nama_kegiatan: '{!! addslashes(old('nama_kegiatan', $kegiatan->nama_kegiatan)) !!}',
                        deskripsi: '{!! addslashes(old('deskripsi', $kegiatan->deskripsi)) !!}'
                    },
                    get characterCount() {
                    ...
                    submitForm() {
                        this.loading = true;
                        this.$refs.form?.submit() || document.getElementById('kegiatanForm').submit();
                    }
                }
            }
        </script>
