{{-- resources/views/warga/remaja/edit.blade.php --}}

@extends('layouts.public')

@section('hide-footer', true)
@section('title', 'Edit Posyandu Remaja - Sistem Informasi Posyandu')

@section('page-title', '🧑 Edit Data Remaja')
@section('page-subtitle', 'Perbarui data identitas remaja untuk layanan Posyandu Remaja.')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-md overflow-hidden">
        <div class="p-6">
            <form method="POST" action="{{ route('warga.remaja.update', $remaja) }}" 
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="alamat" id="alamat" rows="3" 
                                  class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent text-gray-900 dark:text-white">{{ old('alamat', $remaja->alamat) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="golongan_darah" id="golongan_darah" 
                                class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent text-gray-900 dark:text-white">
                            <option value="">Pilih...</option>
                            <option value="A" {{ old('golongan_darah', $remaja->golongan_darah) == 'A' ? 'selected' : '' }}>A</option>
                            <option value="B" {{ old('golongan_darah', $remaja->golongan_darah) == 'B' ? 'selected' : '' }}>B</option>
                            <option value="AB" {{ old('golongan_darah', $remaja->golongan_darah) == 'AB' ? 'selected' : '' }}>AB</option>
                            <option value="O" {{ old('golongan_darah', $remaja->golongan_darah) == 'O' ? 'selected' : '' }}>O</option>
                        </select>
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="flex-shrink-0">
                                <div class="w-20 h-20 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600">
                                    <img id="fotoPreview" 
                                         src="{{ $remaja->foto_url ?? 'https://ui-avatars.com/api/?name=User&color=7F9CF5&background=EBF4FF&size=100' }}" 
                                         alt="Preview" 
...
<script>
    function remajaForm() {
        return {
            loading: false,
            form: {
                nama: '{{ old('nama', $remaja->nama) }}',
                nik: '{{ old('nik', $remaja->nik) }}',
                tanggal_lahir: '{{ old('tanggal_lahir', $remaja->tanggal_lahir?->format('Y-m-d')) }}'
            },
            nikError: null,
            age: null,
